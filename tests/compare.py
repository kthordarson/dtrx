...
if os.path.exists('scripts/dtrx.py') and os.path.exists('tests'):
	os.chdir('tests')
elif os.path.exists('../scripts/dtrx.py') and os.path.exists('../tests'):
	pass
else:
	print ("ERROR: Can't run tests in this directory!")
	sys.exit(2)

DTRX_SCRIPT = os.path.realpath('../scripts/dtrx.py')
SHELL_CMD = ['sh', '-se']
...
class ExtractorTest(object):
	...
	def get_results(self, command, stdin=None):
		# print(f'[get_results] command {command}')
		...
		output = subprocess.getoutput('find')
		# process.stdout.close()
		# process.wait()
		# print(f'[get_result] [status] {type(status)} {status}')
		# print(f"[get_result] [output] {type(output)} {output}")
	...
	def get_extractor_results(self):
		self.run_script('prerun')
		return self.get_results([DTRX_SCRIPT] + self.options + self.filenames, self.input)
	...
	def show_status(self, status, message=None):
		...
		if raw_status != 'passed':
			...
			sys.stdout.write(str(self.outbuffer.read(-1)))
		if message is None:
			last_part = ''
		else:
			last_part = ': %s' % (message,)
	...
	def compare_results(self, actual):
		...
		if expected != actual:
			...
			print ("Posttest gave status code", posttest_result)
			return self.show_status('FAILED')
	...
	def grep_output(self, output):
		for pattern in self.grep:
			if not re.search(pattern.replace(' ', '\\s+'), str(output), re.MULTILINE):
				return "output did not match %s" % (pattern)
		for pattern in self.antigrep:
			if re.search(pattern.replace(' ', '\\s+'), str(output), re.MULTILINE):
...
test_data = yaml.load(test_db.read(-1), Loader=yaml.FullLoader)
test_db.close()
tests = [ExtractorTest(**data) for data in test_data]
for original_data in test_data:
	try:
		#if (original_data.has_key('directory') or (not original_data.has_key('baseline'))):
