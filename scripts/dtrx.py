...
class BaseExtractor(object):
    ...
    def __init__(self, filename, encoding):
        ...
        try:
            ...
            raise ExtractorError(
                f"could not open {filename}: {error}"
        ...
        while True:
            try:
                ...
                if self.pw_prompted and self.ignore_pw:
                    ...
                    self.stderr = ""
                    raise ExtractorError(
                        f"cannot extract encrypted archive {self.filename} in non-interactive mode"
                    )
    ...
    def add_process(self, processes, command, stdin, stdout):
        ...
        try:
            processes.append(
                subprocess.Popen(
                    command, stdin=stdin, stdout=stdout, stderr=self.stderr
                )
            )
        except OSError as error:
    ...
    def check_success(self, got_files):
        ...
        if self.is_fatal_error(error_code) or (
            ...
            raise ExtractorError(
                f"{self.pipes[error_index][1]}, {command}, {error_code}"
            )
    ...
    def extract(self):
        ...
        try:
            ...
            logger.error(
                f"[err] {e} oldpath:{old_path} st:{self.target} {self.archive}"
            )
            self.archive.close()
            os.chdir(old_path)
...
class OneEntryPolicy(BasePolicy):
    ...
    def prep(self, archive_filename, extractor):
        question = self.wrap(
            "%s contains one %s but its name doesnt match.",
            archive_filename,
...
class ExtractorBuilder(object):
    ...
    def magic_map_matches(self, cls=None, output=None, magic_map=None):
        return [
            result
            for regexp, result in list(magic_map.items())
...
class ExtractorApplication(object):
    ...
    def abort(self, signal_num, frame):
        ...
        # print('')
        logger.debug(
            f"traceback: {traceback.format_stack(frame)} sig:{signal_num}"
    ...
    def parse_options(self, arguments):
        ...
        parser.add_option(
            ...
            action="store_true",
            default=False,
            help="dont ask how to handle special cases",
        )
        parser.add_option(
            "-o",
            "--overwrite",
    ...
    def try_extractors(self, filename, builder):
        ...
        for extractor in builder:
            ...
            if error:
                logger.warning(
                    f"[tryext] {error} extr:{extractor} b:{builder} fn:{filename}"
                )
                errors.append(
                    (
                        extractor.file_type,
                ...
                logger.info(
                    f"[tryext] {filename} extracted with {extractor.file_type} {extractor.encoding} {error}"
                )
    ...
    def run(self):
        ...
        while self.archives:
            ...
            logger.debug(
                f"[run] a:{self.action} cwd:{self.current_directory} fns:{self.filenames}"
            )
